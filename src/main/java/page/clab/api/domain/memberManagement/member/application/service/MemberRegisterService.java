...
public class MemberRegisterService implements RegisterMemberUseCase {
     ...
     * 비밀번호가 입력되지 않은 경우 새 비밀번호를 생성하거나, 입력된 비밀번호를 사용합니다. 등록이 완료되면 기본 직책(Position)을 생성하며, 최종적으로 생성된 계정 정보를 이메일을 통해 사용자에게
     * 전송합니다.</p>
     *
    ...
    public void createPositionByMember(Member member) {
        if (externalRetrievePositionUseCase.findByMemberIdAndYearAndPositionType(member.getId(),
            String.valueOf(LocalDate.now().getYear()), PositionType.MEMBER).isPresent()) {
            return;
        }
